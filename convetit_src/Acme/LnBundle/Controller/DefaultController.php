...
namespace Acme\LnBundle\Controller;

use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\HttpFoundation\RedirectResponse;
use Acme\LnBundle\Entity\Users;
...
/* class DefaultController extends Controller
{
    ...
    {
        return $this->render('AcmeLnBundle:Default:index.html.twig', array('name' => $name));
    }
}
 */
/**
...
class DefaultController extends Controller {
     ...
     * @param array $linkedIn_oauth array of user oauth data
     */
     public function getUserInfo($appKey, $appSecret, $linkedIn_oauth) {
	    //linkedIn config parameters
        $config = array('appKey' => $appKey,
            'appSecret' => $appSecret,
            'callbackUrl' => '');
        ...
        $oauth->setTokenAccess($linkedIn_oauth);
        $userData = $oauth->profile('~:(id,first-name,last-name,picture-url,headline,site-standard-profile-request,location:(country:(code)),summary,positions,skills,educations,courses,email-address,phone-numbers,main-address,date-of-birth)');
			//check if connection success with linked in
		...
		if (200 == $userData['info']['http_code']) {
			 ...
			 $xml = simplexml_load_string($userData['linkedin']);
			...
			$user_data_arr['id']=$xml->id;
			$user_data_arr['first_name']=$xml->{'first-name'};
			$user_data_arr['last_name']=$xml->{'last-name'};
			$user_data_arr['headline']=$xml->headline;
			$user_data_arr['email_address']=$xml->{'email-address'};
			$user_data_arr['picture_url']=$xml->{'picture-url'};
			$user_data_arr['bio']=$xml->{'summary'};
			

			$repository = $this->getDoctrine()
			->getRepository('AcmeLnBundle:Users');

			$query = $repository->createQueryBuilder('u')
			->where('u.api_id = :api_id')
			->setParameter('api_id', $user_data_arr['id'])
			->getQuery();
			...
			if(count($users))
			{
				if($users[0]->getApiId()!= $user_data_arr['id'])
				{
					$is_insert=1;
				}
			...
			$users = new Users();
			$users->setFirstname($user_data_arr['first_name']);
			$users->setLastname($user_data_arr['last_name']);
			$users->setEmail($user_data_arr['email_address']);
			$users->setApiId($user_data_arr['id']);
			$users->setHeadline($user_data_arr['headline']);
		
			$users->setDlu(new \DateTime());
			$users->setDoe(new \DateTime());
			$users->setStatus('1');
			$users->setBio($user_data_arr['bio']);
			$users->setProfileImage($user_data_arr['picture_url']);
			...
			/* else
			{
			 ...
			 return $this->render('AcmeLnBundle:Default:welcome_user.html.twig', array('user_info'=>$user_data_arr));
          } else {
            return NULL;
        }
    ...
    public function linkedInCallBackAction() {
        ...
        if (200 == $access_token['info']['http_code']) {
         ...
         //   return $this->redirect($this->generateUrl('handleLnResponse', array('access_token' => $access_token), TRUE));
		return	$this->getUserInfo($this->container->getParameter('linkedin_api_key'),$this->container->getParameter('linkedin_secret_key'), $oauth->getToken());
